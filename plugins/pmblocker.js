...
export async function before(m, { conn, isAdmin, isBotAdmin, isOwner, isROwner }) {
    ...
    if (!m.isGroup) {
      ...
      if (bot.pmblocker && !isOwner && !isROwner) {
        ...
        await m.reply(`*Hey @${m.sender.split`@`[0]}, messaging the 🤍 MAGIC MOA 🤍─❂  bot privately is currently disabled. You have been blocked from using the 🤍 MAGIC MOA 🤍─❂ bot.*`, false, { mentions: [m.sender] });
