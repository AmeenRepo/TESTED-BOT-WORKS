...
export async function Moa(chatUpdate) {
    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (xp > 200)
                    m.reply('*_Hehe 👀_*') // Hehehe
                else
                    m.exp += xp
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                     this.reply(m.chat, `Magic-Moa 🕊️ Money Finished \n Use The Following Magic-Moa 🕊️ Command To Buy Money \n*${usedPrefix}todiamond* <amount`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `Magic-Moa 🕊️ Required Level ${plugin.level} To Use This Magic-moa 🕊️ Command. \nYour Level ${_user.level}`, m)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`*Magic-Moa 🕊️ Plugin:* ${m.plugin}\n*Magic-Moa 🕊️ Sender:* ${m.sender}\n*Magic-Moa 🕊️ Chat:* ${m.chat}\n*Magic-Moa 🕊️ Command:* ${usedPrefix}${command} ${args.join(' ')}\nMagic-Moa 🕊️ *Error Logs:*\n\n\`\`\`${text}\`\`\``.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
...
export async function participantsUpdate({ id, participants, action }) {
   ...
   case 'add':
            if (chat.welcome) {
              ...
              for (let user of participants) {
                ...
                try {
                  ...
                  console.error(`Error  profile picture: ${error}`);
                  pp = 'https://i.imgur.com/9M9vfFm.jpeg';
                  ppgp = 'https://i.imgur.com/9M9vfFm.jpeg'; 
                } finally {
                  let text = (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user')
                    .replace('@group', await this.getName(id))
                    .replace('@desc', groupMetadata.desc?.toString() || 'Discription')
                    .replace('@user', '@' + user.split('@')[0]);
          
                  let nthMember = groupMetadata.participants.length;
                  let secondText = `*_Welcome_* ${await this.getName(user)} *_Our_* *_${nthMember}th member_*`;
		...
		case 'remove':
            if (chat.welcome) {
              ...
              for (let user of participants) {
                ...
                try {
                  ...
                  let text = (chat.sBye || this.bye || conn.bye || '*_HELLO✌🏻, @user_*')
                    .replace('@user', '@' + user.split('@')[0]);
          
                  let nthMember = groupMetadata.participants.length;
                  let secondText = `*_Happy Journey 👀, ${nthMember}th Member_*`;
            ...
            break;    
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user *_Promoted To Admin👥_*')
        case 'demote':
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user *_Is Not An Admin Now👥_*')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || '*_Group Icon Changed By Magic-Moa ✅_*').replace('@icon', groupUpdate.icon)
...
≡  Magic-Moa 🕊️ WA BOT deleted A Message.
┌─⊷ ★ 𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀 ★
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*ᴏɴʟʏ Magic-Moa 🕊️ ᴅᴇᴠᴇʟᴏᴘᴇʀ* • ★ This command can only be used by the *Creator of 𝞓𝞛𝞢𝞢𝞜-𝙎𝞢𝞒*',
        owner: '*ᴏɴʟʏ Magic-Moa 🕊️ ᴏᴡɴᴇʀ* • ★ This command can only be used by the *Bot Owner 𝞓𝞛𝞢𝞢𝞜-𝙎𝞢𝞒',
        mods: '*ᴏɴʟʏ Magic-Moa 🕊️ ᴍᴏᴅᴇʀᴀᴛᴏʀ* •★ This function is only for *For Magic-Moa 🕊️ Bot moderators*',
        premium: '*ᴏɴʟʏ Magic-Moa 🕊️ ᴘʀᴇᴍɪᴜᴍ User* • ★ This command is for *Magic-Moa 🕊️ Premium members only',
        group: '*Magic-Moa 🕊️ ɢʀᴏᴜᴘ ᴄʜᴀᴛ* • ★ This command can only be used in Magic-Moa 🕊️ groups',
        private: '*Magic-Moa 🕊️ ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ* • ★ This command can only be used in the *private chat of the Magic-Moa 🕊️ Bot*',
        admin: '*ᴏɴʟʏ Magic-Moa 🕊️ ᴀᴅᴍɪɴ* • ★ This command is only for *Magic-Moa 🕊️ Group Admins*',
        botAdmin: '*ᴏɴʟʏ Magic-Moa 🕊️ ʙᴏᴛ ᴀᴅᴍɪɴ* • ★ To use this command I must be *Magic-Moa 🕊️ Admin!*',
        unreg: '*ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ʀᴇɢɪsᴛᴇʀᴇᴅ with Magic-Moa 🕊️* • ★ Sign in to Magic-Moa 🕊️,use this feature Type do it :\n\n*/reg name.age*\n\n★ Example : *.reg Ameen.Ser*', 
        restrict: '*ʀᴇsᴛʀɪᴄᴛby Magic-Moa 🕊️ WA BOT* • This feature is *disabled*by Magic-Moa 🕊️ WA BOT',
    }[type]
...
watchFile(file, async () => {
    unwatchFile(file)
    console.log(chalk.redBright("Update 'Moa.js'"))
    if (global.reloadMoa) console.log(await global.reloadMoa())
})
